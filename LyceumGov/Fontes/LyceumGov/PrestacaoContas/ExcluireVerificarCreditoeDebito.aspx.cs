...
{
    [NavUrl("~/PrestacaoContas/ExcluireVerificarCreditoeDebito.aspx"), ControlText("Excluir Operações"), Title("Excluir Operações")]
    public partial class ExcluireVerificarCreditoeDebito : TPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    TxtValor2.Text = operacao.Valor.ToString();
                    TxtDataEnvio.Text = operacao.DataCadastro.ToString();
        ...
        protected void btnCancel_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                Response.Redirect("ExcluirCreditoeDebito.aspx");
            }
            catch (Exception ex)
            {
                lblMensagem.Text = ex.Message;
        ...
        protected void btnConfirmarExclusao_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                rnOperacao.ExcluiOperacao(Convert.ToInt32(hdnOcorrenciaId.Value), TxtMotivoExclusao.Text);

                Response.Redirect("ExcluirCreditoeDebito.aspx?tipo=1");
        ...
        protected void btnExcluir_Click(object sender, EventArgs e)
        {
            try
            {
                var verificaenviosei = rnOperacao.VerificaEnvioSEI(Convert.ToInt32(hdnOcorrenciaId.Value));
                if (verificaenviosei)
                    throw new Exception("Não é possível excluir este débito/crédito porque o Formulário SEI já foi gerado");

                if (TxtMotivoExclusao.Text != "")
                {
                    this.Page.ClientScript.RegisterStartupScript(Page.GetType(), "popup", "abrirPopup();", true);
                }
                else
                {
                    this.Page.ClientScript.RegisterStartupScript(this.Page.GetType(), "popup", "alert('Motivo da Exclusão não preenchido!');", true);
                }
            }
            catch (Exception ex)
            {
