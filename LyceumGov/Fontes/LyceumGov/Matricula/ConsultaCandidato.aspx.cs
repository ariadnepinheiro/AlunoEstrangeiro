...
{
    [NavUrl("~/Matricula/ConsultaCandidato.aspx")]
    [ControlText("ConsultaCandidato")]
    [Title("Consulta Candidato")]
    public partial class ConsultaCandidato : TPage
    {
        ...
        private void LimparDadosBusca()
        {
            ...
            chkNaoDeclarPaiBusca.Checked = false;
            txtCPFResponsavel.Text = string.Empty;
            chkFonetizarCandidato.Checked = false;
        }
        ...
        protected void btnBuscarCandidato_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                RN.Matriculas.InscricaoAluno rnInscricao = new Techne.Lyceum.RN.Matriculas.InscricaoAluno();
                ...
                {
                    if (!txtNomeBusca.Text.IsNullOrEmptyOrWhiteSpace() || !txtNomeMae.Text.IsNullOrEmptyOrWhiteSpace() || !dtDataNasc.Text.IsNullOrEmptyOrWhiteSpace() || !txtNumInscricao.Text.IsNullOrEmptyOrWhiteSpace() || !txtCPFResponsavel.Text.IsNullOrEmptyOrWhiteSpace() || !txtPai.Text.IsNullOrEmptyOrWhiteSpace())
                    {
                        dados = rnInscricao.ObtemDadosCandidatoPor(txtNomeBusca.Text, txtNomeMae.Text, (!dtDataNasc.Text.IsNullOrEmptyOrWhiteSpace() ? dtDataNasc.Date : (DateTime?)null), Convert.ToInt32(ddlAno.SelectedValue), (!ddlPeriodo.SelectedValue.IsNullOrEmptyOrWhiteSpace() ? Convert.ToInt32(ddlPeriodo.SelectedValue) : -1), txtNumInscricao.Text.Trim(), txtPai.Text, txtCPFResponsavel.Text.RetirarMascaraCPF(), chkFonetizarCandidato.Checked);
                        ...
                        {
                            lblMensagem.Text = "Candidato não localizado com os dados informados. Verifique!";
                        }
                    }
                    else
                    {
                        lblMensagem.Text = "Para efetuar a busca é necessário escolher outros campos no filtro.";
                        return;
                    }
                }
                else
                {
                    lblMensagem.Text = "Para efetuar a busca é necessário informar o Ano.";
                    return;
                }
            }
            catch (Exception ex)
        ...
        protected void chkNaoDeclarPaiBusca_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (chkNaoDeclarPaiBusca.Checked)
                {
                    txtPai.Text = chkNaoDeclarPaiBusca.Text.ToUpper();
        ...
        protected void chkFonetizarCandidato_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
