...
public class registrosremessa : System.Web.Services.WebService
{

    public registrosremessa()
    {
    ...
    {
        registroRemessaResponse registro = new registroRemessaResponse();

        SqlConnection conexao = new SqlConnection(ConfigurationManager.ConnectionStrings["conexaoSQL"].ConnectionString);

        string sql1 = "SELECT distinct nomeArquivo, quantidaderegistros FROM RIOCARD_REMESSA where nomearquivo = '" + nomeArquivo + "' AND nomearquivo like 'SLNSEE%'";

        string sql = "SELECT tipomovimentacao, matriculanova, nomealuno, alunodatanascimento, " +
                     ...
                     "codigocenso, qhiturno, qhiturma, alunofoto, gratuidade, modalTREM, modalONIBUS, modalMETRO, modalBARCAS from riocard_remessa where nomearquivo = '" + nomeArquivo + "' AND nomearquivo like 'SLNSEE%'";
        ...
        try
        {
            ...
            while (reader.Read())
            {
                ...
                registroRemessaResponse.registro registroAux = new registroRemessaResponse.registro();
                ...
                {
                    DateTime alunoDataNascimento = DateTime.MinValue;
