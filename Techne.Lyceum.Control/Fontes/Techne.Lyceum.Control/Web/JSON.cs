...
namespace Techne.Web
{
    internal class JSON
    {
        ...
        public static string GetLastErrorSnippet()
        {
            ...
            {
                var startIndex = JSON.instance.lastErrorIndex - 5;
                var endIndex = JSON.instance.lastErrorIndex + 15;
                if (startIndex < 0)
                {
                    startIndex = 0;
                }
        ...
        public static object JsonDecode(string json)
        {
            ...
            var chArray = json.ToCharArray();
            var index = 0;
            var success = true;
            var obj2 = instance.ParseValue(chArray, ref index, ref success);
            if (success)
            {
        ...
        {
            var builder = new StringBuilder(0x7d0);
        ...
        {
            var saveIndex = index;
            return this.NextToken(json, ref saveIndex);
        }
        ...
        protected ArrayList ParseArray(char[] json, ref int index)
        {
            ...
            while (!done)
            {
                ...
                {
                    var success = true;
                    var value = this.ParseValue(json, ref index, ref success);
                    if (!success)
                    {
                        return null;
                    }
        ...
        protected double ParseNumber(char[] json, ref int index)
        {
            ...
            var lastIndex = this.GetLastIndexOfNumber(json, index);
        ...
        protected Hashtable ParseObject(char[] json, ref int index)
        {
            ...
            while (!done)
            {
                token = this.LookAhead(json, index);
                if (token == JSON.TOKEN_NONE)
                {
                    return null;
                }
                ...
                {
                    // name
                    var name = this.ParseString(json, ref index);
                    if (name == null)
                    {
                        return null;
                    }
...
// skip 4 chars
        ...
        protected bool SerializeObjectOrArray(object objectOrArray, StringBuilder builder)
        {
            ...
            {
                return this.SerializeArray((ArrayList)objectOrArray, builder);
            }
            else
            {
                return false;
