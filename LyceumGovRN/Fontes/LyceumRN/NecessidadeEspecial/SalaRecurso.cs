...
namespace Techne.Lyceum.RN.NecessidadeEspecial
{
    public class SalaRecurso
    {
        public DateTime[] RetornaDataMinimaMaximaPor(DataContext contexto, string aluno)
        {
            ...
            try
            {
                contextQuery.Command = @" SELECT MIN(DATAINICIO) AS DATAMINIMA, 
                                            ...
                                            FROM   NECESSIDADEESPECIAL.SALARECURSO T (NOLOCK) 
        ...
        public bool EhInterpreteAtivoPor(DataContext contexto, int recursoNecessidadeEspecialId)
        {
            ...
            try
            {
                contextQuery.Command = @" SELECT COUNT(*) 
                                    FROM   NECESSIDADEESPECIAL.SALARECURSO (NOLOCK) 
        ...
        public DataTable ListaPor(string aluno)
        {
            ...
            try
            {
                contextQuery.Command = @" SELECT DISTINCT SALARECURSOID, 
	                               M.ALUNO, 
	                               T.TURMA,
        ...
        public DataTable ListaPorCpf(string cpf)
        {
            ...
            try
            {
                contextQuery.Command = @" SELECT DISTINCT SALARECURSOID, 
                                               ...
                                               T.ANO,
                                               T.SEMESTRE
                                        FROM   NECESSIDADEESPECIAL.SALARECURSO LA (NOLOCK) 
        ...
        public ValidacaoDados Valida(Entidades.SalaRecurso salarecurso, bool cadastro, string unidadeEnsino, string curso, string codigoTurno, decimal serie, string cpf)
        {
            ...
            if (!cadastro)
            {
                if (salarecurso.SalaRecursoId <= 0)
                {
                    mensagens.Add("Campo CÓDIGO é obrigatório.");
                }
            }

            if (salarecurso.UsuarioId.IsNullOrEmptyOrWhiteSpace())
            {
            ...
            if (salarecurso.RecursoNecessidadeEspecialId <= 0)
            {
            ...
            if (cadastro)
            {
                ...
                if (salarecurso.Ano <= 0)
                {
                    mensagens.Add("Campo ANO LETIVO é obrigatório.");
                }
                if (salarecurso.Semestre < 0)
                {
                ...
                if (salarecurso.Turma.IsNullOrEmptyOrWhiteSpace())
                {
                    mensagens.Add("Campo TURMA é obrigatório.");
                }
            }
            if (salarecurso.DataInicio == DateTime.MinValue)
            {
                mensagens.Add("Campo DATA INICIO é obrigatório.");
            }

            if (salarecurso.DataFim == DateTime.MinValue)
            {
                mensagens.Add("Campo DATA FIM é obrigatório.");
            }

            if (salarecurso.DataInicio != DateTime.MinValue && salarecurso.DataFim != DateTime.MinValue && salarecurso.DataInicio > salarecurso.DataFim)
            {
            ...
            if (mensagens.Count == 0)
            {
                try
                {
                    ...
                    if (!rnRecursoNecessidadeEspecial.EhRecursoAtivoPor(contexto, salarecurso.RecursoNecessidadeEspecialId))
                    {
                        mensagens.Add("Este recurso se encontra DESATIVADO.");
                    }

                    //Verifica se o recurso pode atuar como interprete
                    if (!rnRecursoNecessidadeEspecial.PossuiTipoCadastradoPor(contexto, salarecurso.RecursoNecessidadeEspecialId, (int)TipoRecursoNecessidadeEspecial.TipoRecurso.Interprete))
                    {
                    ...
                    else
                    {
                        ...
                        dadosTurma = rnTurma.ObtemDadosTurmaAbertaPor(contexto, salarecurso.Turma, salarecurso.Ano, salarecurso.Semestre);
                        ...
                        else
                        {
                            ...
                            if (!rnAvaliacaoNapes.PossuiAvaliacaoPor(contexto, salarecurso.Turma, salarecurso.Ano, salarecurso.Semestre, (int)TipoRecursoNecessidadeEspecial.TipoRecurso.Interprete))
                            {
                            ...
                            if (!rnNecessidadeEspecial.NecessitaInterpretePor(contexto, salarecurso.Turma, salarecurso.Ano, salarecurso.Semestre))
                            {
                            ...
                            if (dadosTurma.DataFimTurma < salarecurso.DataFim.Date)
                            {
                            ...
                            if (this.PossuiOutroInterpreteAtivoPor(contexto, salarecurso.Turma, salarecurso.DataInicio, salarecurso.DataFim, salarecurso.SalaRecursoId, salarecurso.Ano, salarecurso.Semestre))
                            {
                            ...
                            if (cpf != "00000000000")
                            {
                                ...
                                foreach (string turno in rnCuidadorAluno.ListaTurnoCuidadorAtivoPor(contexto, salarecurso.RecursoNecessidadeEspecialId, salarecurso.DataInicio, salarecurso.DataFim))
                                {
                                ...
                                foreach (string turno in rnLedorAluno.ListaTurnoLedorAtivoPor(contexto, salarecurso.RecursoNecessidadeEspecialId, salarecurso.DataInicio, salarecurso.DataFim))
                                {
                                    string[] turnoTipo = new string[2];
                                    turnoTipo[0] = turno;
                                    turnoTipo[1] = "Ledor";
                                ...
                                foreach (string turno in this.ListaTurnoInterpreteAtivoPor(contexto, salarecurso.RecursoNecessidadeEspecialId, salarecurso.DataInicio, salarecurso.DataFim, salarecurso.Turma))
                                {
                                    string[] turnoTipo = new string[2];
                                    turnoTipo[0] = turno;
            ...
            try
            {
                contextQuery.Command = @" SELECT COUNT(*) 
                                            ...
                                            WHERE  TURMA = @TURMA 
                                                    ...
                                                    AND SALARECURSOID <> @SALARECURSOID
                                                    AND ( @DATAINICIO BETWEEN DATAINICIO AND DATAFIM 
                                                            OR @DATAFIM BETWEEN DATAINICIO AND DATAFIM ) ";
                ...
                contextQuery.Parameters.Add("@SALARECURSOID", SqlDbType.Int, interpreteId);
                contextQuery.Parameters.Add("@DATAINICIO", SqlDbType.Date, dataInicio.Date);
                contextQuery.Parameters.Add("@DATAFIM", SqlDbType.Date, dataFim.Date);
        ...
        public void Insere(Entidades.SalaRecurso interprete)
        {
            ...
            try
            {
                contextQuery.Command = @" INSERT INTO NECESSIDADEESPECIAL.SALARECURSO 
        ...
        public void Atualiza(Entidades.SalaRecurso interprete)
        {
            DataContext contexto = DataContextBuilder.FromLyceum.UsingLock();
            ContextQuery contextQuery = new ContextQuery();
            ...
            try
            {
                contextQuery.Command = @" UPDATE NECESSIDADEESPECIAL.SALARECURSO 
                                                SET    DATAINICIO = @DATAINICIO, 
                                                        DATAFIM = @DATAFIM, 
                                                        USUARIOID = @USUARIOID, 
                                                        DATAALTERACAO = @DATAALTERACAO 
                                                WHERE  SALARECURSOID = @SALARECURSOID  ";

                contextQuery.Parameters.Add("@DATAINICIO", SqlDbType.Date, interprete.DataInicio.Date);
                contextQuery.Parameters.Add("@DATAFIM", SqlDbType.Date, interprete.DataFim.Date);
                contextQuery.Parameters.Add("@USUARIOID", SqlDbType.VarChar, interprete.UsuarioId);
                contextQuery.Parameters.Add("@DATAALTERACAO", SqlDbType.DateTime, DateTime.Now);
                contextQuery.Parameters.Add("@SALARECURSOID", SqlDbType.Int, interprete.SalaRecursoId);
