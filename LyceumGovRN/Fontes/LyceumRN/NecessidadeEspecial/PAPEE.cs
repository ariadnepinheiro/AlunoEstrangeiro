...
namespace Techne.Lyceum.RN.NecessidadeEspecial
{
    public class PAPEE
    {
        public DateTime[] RetornaDataMinimaMaximaPor(DataContext contexto, string aluno)
        {
            ...
            try
            {
                contextQuery.Command = @" SELECT MIN(DATAINICIO) AS DATAMINIMA, 
                                            ...
                                            FROM   NECESSIDADEESPECIAL.PAPEE T (NOLOCK) 
        ...
        public bool EhInterpreteAtivoPor(DataContext contexto, int recursoNecessidadeEspecialId)
        {
            ...
            try
            {
                contextQuery.Command = @" SELECT COUNT(*) 
                                    FROM   NECESSIDADEESPECIAL.PAPEE (NOLOCK) 
        ...
        public DataTable ListaPor(string aluno)
        {
            ...
            try
            {
                contextQuery.Command = @" SELECT DISTINCT PAPEEID, 
	                               M.ALUNO, 
	                               T.TURMA,
        ...
        public DataTable ListaPorCpf(string cpf)
        {
            ...
            try
            {
                contextQuery.Command = @" SELECT DISTINCT PAPEEID, 
                                               ...
                                               T.ANO,
                                               T.SEMESTRE
                                        FROM   NECESSIDADEESPECIAL.PAPEE LA (NOLOCK) 
        ...
        public ValidacaoDados Valida(Entidades.PAPEE interprete, bool cadastro, string unidadeEnsino, string curso, string codigoTurno, decimal serie, string cpf)
        {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (this.PossuiOutroInterpreteAtivoPor(contexto, interprete.Turma, interprete.DataInicio, interprete.DataFim, interprete.PAPEEId, interprete.Ano, interprete.Semestre))
                            {
        ...
        private bool PossuiOutroInterpreteAtivoPor(DataContext ctx, string turma, DateTime dataInicio, DateTime dataFim, int interpreteId, int ano, int semestre)
        {
            ...
            try
            {
                ...
                contextQuery.Parameters.Add("@PAPEEID", SqlDbType.Int, interpreteId);
                contextQuery.Parameters.Add("@DATAINICIO", SqlDbType.Date, dataInicio.Date);
                contextQuery.Parameters.Add("@DATAFIM", SqlDbType.Date, dataFim.Date);
        ...
        public void Insere(Entidades.PAPEE interprete)
        {
            ...
            try
            {
                contextQuery.Command = @" INSERT INTO NECESSIDADEESPECIAL.PAPEE 
        ...
        public void Atualiza(Entidades.PAPEE interprete)
        {
            DataContext contexto = DataContextBuilder.FromLyceum.UsingLock();
            ContextQuery contextQuery = new ContextQuery();
            ...
            try
            {
                contextQuery.Command = @" UPDATE NECESSIDADEESPECIAL.PAPEE 
                                                SET    DATAINICIO = @DATAINICIO, 
                                                        DATAFIM = @DATAFIM, 
                                                        USUARIOID = @USUARIOID, 
                                                        DATAALTERACAO = @DATAALTERACAO 
                                                WHERE  PAPEEID = @PAPEEID  ";

                contextQuery.Parameters.Add("@DATAINICIO", SqlDbType.Date, interprete.DataInicio.Date);
                contextQuery.Parameters.Add("@DATAFIM", SqlDbType.Date, interprete.DataFim.Date);
                contextQuery.Parameters.Add("@USUARIOID", SqlDbType.VarChar, interprete.UsuarioId);
                contextQuery.Parameters.Add("@DATAALTERACAO", SqlDbType.DateTime, DateTime.Now);
                contextQuery.Parameters.Add("@PAPEEID", SqlDbType.Int, interprete.PAPEEId);
