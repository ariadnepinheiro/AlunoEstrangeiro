...
namespace Techne.Lyceum.RN.Turmas
{
    public class FrequenciaTurma
    {
        ...
        public ValidacaoDados Valida(Entidades.FrequenciaTurma frequenciaTurma, List<Entidades.FrequenciaAluno> frequenciaAluno, List<string> alunos, bool historico)
        {
            ...
            RN.Turmas.FrequenciaAluno rnFrequenciaAluno = new FrequenciaAluno();
            ...
            if (mensagens.Count == 0)
            {
                try
                {
                    ...
                    {
                        bool matriculaAtiva = rnMatricula.PossuiMatriculaAtivaNaTurmaPorAluno(contexto, aluno, frequenciaTurma.Turma, frequenciaTurma.Ano, frequenciaTurma.Periodo);

                        //Verifica se está vindo da tela normal e se o aluno não continua com matricula ativa
                        if (!historico && !matriculaAtiva)
                        {
                            mensagens.Add(string.Format("O aluno {0} não está mais com a matricula Ativa nesta turma. Favor realizar nova busca.", aluno));
                        }

                        //Verifica se está vindo da tela historico e se o aluno esta oom matricula ativa
                        if (historico && matriculaAtiva)
                        {
                            mensagens.Add(string.Format("O aluno {0} está a matricula Ativa nesta turma. Favor realizar nova busca.", aluno));
                        }
                    }
                    ...
                    foreach ( Entidades.FrequenciaAluno aluno in frequenciaAluno)
                    {
                        string turma = rnFrequenciaAluno.ObtemTurmaComLancamentoPor(contexto, aluno.Aluno, aluno.DataAusencia, frequenciaTurma.Turma);

                        if (!turma.IsNullOrEmptyOrWhiteSpace())
                        {
                            mensagens.Add(string.Format("O aluno {0} está com falta lançada no dia {1} em outra turma, logo não pode ter falta nesta turma. Favor verificar.", aluno.Aluno, aluno.DataAusencia.ToString("dd/MM/yyyy")));
        ...
        private void Insere(DataContext contexto, Entidades.FrequenciaTurma frequenciaTurma)
        {
            ...
            contextQuery.Command = @" INSERT INTO Turma.FREQUENCIATURMA
                                           (CENSO
                                           ,ANO
                                           ,PERIODO
                                           ,TURMA
                                    ...
                                    SELECT IDENT_CURRENT('Turma.FREQUENCIATURMA') ";
        ...
        private void Atualiza(DataContext contexto, Entidades.FrequenciaTurma frequenciaTurma)
        {
            ...
            contextQuery.Command = @" UPDATE Turma.FREQUENCIATURMA
