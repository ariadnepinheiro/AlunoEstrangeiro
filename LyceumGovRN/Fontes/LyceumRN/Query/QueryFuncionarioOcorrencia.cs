...
{
    public class QueryFuncionarioOcorrencia : LyceumQuery
    {
        public QueryFuncionarioOcorrencia()
            : base()
        {
            ...
            this.GridColumns.Add(new TSearchColumn() { FieldName = "cargo", Caption = "Cargo", Width = Unit.Percentage(0), Visible = true });
            this.GridColumns.Add(new TSearchColumn() { FieldName = "nomefuncao", Caption = "NomeFuncao", Width = Unit.Percentage(0), Visible = true });
            this.GridColumns.Add(new TSearchColumn() { FieldName = "funcao", Caption = "Funcao", Width = Unit.Percentage(0), Visible = true });
        ...
        public override DataView GetData(IDictionary<string, object> pars, object key, int maxRows)
        {
            ...
            sql.Append("SELECT distinct top " + maxRows.ToString() + @" p.matricula,
                        ...
                        LEFT JOIN LY_FUNCAO F ON F.FUNCAO = P.FUNCAO
                        WHERE  ? BETWEEN DATA_NOMEACAO AND ISNULL(DATA_DESATIVACAO, GETDATE()) ");

            parValues.Add(this.HasValue(pars, "DataOcorrencia") ? Convert.ToDateTime(pars["DataOcorrencia"]).ToString("yyyy-MM-dd") : DateTime.Now.ToString("yyyy-MM-dd"));
