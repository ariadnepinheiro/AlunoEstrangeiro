...
{
    public class QueryUsuarioExterno: LyceumQuery
    {
        public QueryUsuarioExterno()
        {
            this.GridColumns.Add(new TSearchColumn() { FieldName = "cpf", Caption = "CPF", Width = Unit.Percentage(10) });
			this.GridColumns.Add(new TSearchColumn() { FieldName = "nome", Caption = "Nome", Width = Unit.Percentage(60) });
            this.GridColumns.Add(new TSearchColumn() { FieldName = "codigo", Caption = "Código", Width = Unit.Percentage(10) });  
            ...
            this.GridColumns.Add(new TSearchColumn() { FieldName = "celular", Caption = "celular", Width = Unit.Percentage(0), Visible = false });
            ...
            this.GridFilterParameters.Add("email", "E-mail", TSearchDataType.String, 19);
            this.MaxRows = 100;
            this.MaxLength = 100;
        ...
        public override DataView GetData(IDictionary<string, object> pars, object key, int maxRows)
        {
            ...
            try
            {
                ...
                {
                    contextQueryPar.Command += " AND P.E_MAIL_INTERNO like @email ";
                    contextQuery.Parameters.Add("@email ", this.LikeExpression(pars["email"].ToString()));
                }

                if (pars.ContainsKey("tipoUsuarioExterno") && pars["tipoUsuarioExterno"] != null && pars["tipoUsuarioExterno"].ToString().Trim().Length > 0)
                {
                    contextQueryPar.Command += " AND ue.TIPOUSUARIOEXTERNOID = @tipoUsuarioExterno ";
                    contextQuery.Parameters.Add("@tipoUsuarioExterno ", pars["tipoUsuarioExterno"].ToString());
                }
                else 
                {
                    contextQueryPar.Command += " AND ue.TIPOUSUARIOEXTERNOID is null ";
                }

                contextQuery.Command = @" SELECT DISTINCT TOP ( 50 ) P.NOME_COMPL AS NOME,
                                                              P.CPF,
                                                              P.PESSOA,
															  P.FONE,
															  P.CELULAR,
