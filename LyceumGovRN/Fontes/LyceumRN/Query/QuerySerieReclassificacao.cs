...
{
    public class QuerySerieReclassificacao : LyceumQuery
    {
        public QuerySerieReclassificacao() : base()
        {
            ...
            this.GridFilterParameters.Add(new TSearchParameter() { Caption = "Ano de Ingresso", ParameterName = "ano_ingresso", ShowInFilterPanel = false, ParameterType = TSearchDataType.String });
            this.GridFilterParameters.Add(new TSearchParameter() { Caption = "Periodo de Ingresso", ParameterName = "periodo_ingresso", ShowInFilterPanel = false, ParameterType = TSearchDataType.String });
            ...
            this.GridFilterParameters.Add(new TSearchParameter() { Caption = "Matriz Curricular", ParameterName = "curriculo", ShowInFilterPanel = false, ParameterType = TSearchDataType.String });
        ...
        public override DataView GetData(IDictionary<string, object> pars, object key, int maxRows)
        {
            ...
            else if (pars.ContainsKey("ano_ingresso") && pars["ano_ingresso"] != null && pars["ano_ingresso"].ToString().Trim().Length > 0)
            {
                sql.Append(" and cur.ANO_INI = ? ");
                parValues.Add(pars["ano_ingresso"].ToString());
            }
            ...
            else if (pars.ContainsKey("periodo_ingresso") && pars["periodo_ingresso"] != null && pars["periodo_ingresso"].ToString().Trim().Length > 0)
            {
                sql.Append(" and cur.SEM_INI = ? ");
                parValues.Add(pars["periodo_ingresso"].ToString());
            } 
            ...
            if (pars.ContainsKey("curriculo") && pars["curriculo"] != null && pars["curriculo"].ToString().Trim().Length > 0)
            {
                sql.Append(" and s.curriculo = ? ");
                parValues.Add(pars["curriculo"].ToString());
            }
