...
{
    public class QueryDocenteResetSenha : LyceumQuery
    {
        public QueryDocenteResetSenha()
        {
            ...
            this.GridColumns.Add(new TSearchColumn() { FieldName = "email", Caption = "E-mail", Width = Unit.Percentage(18) });
            ...
            this.GridFilterParameters.Add("email", "E-mail", TSearchDataType.String, 15);
        ...
        public override DataView GetData(IDictionary<string, object> pars, object key, int maxRows)
        {
            ...
            sql.Append("SELECT top " + maxRows.ToString() + "d.matricula, d.num_func, p.nome_compl as nome,  p.rg_num, p.cpf, p.pessoa, " +
               "p.nome_mae as mae, " +
               "p.nome_pai as pai, " +
               "p.E_MAIL_INTERNO as email, p.idfuncional, ISNULL((convert(varchar,p.IDFUNCIONAL) + '/' + convert(varchar,d.VINCULO)),d.matricula) idvinculo FROM  " +
               "ly_pessoa p (nolock) inner join ly_docente d " +
               "on p.pessoa = d.pessoa where voluntario='N'");
            ...
            {
                sql.Append(" and p.E_MAIL_INTERNO like ? ");
                parValues.Add(LikeExpression(pars["email"].ToString()));
            }
