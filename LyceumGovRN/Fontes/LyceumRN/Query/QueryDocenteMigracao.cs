...
{
    public class QueryDocenteMigracao : LyceumQuery
    {
        public QueryDocenteMigracao()
        {
            ...
            this.GridColumns.Add(new TSearchColumn() { FieldName = "idade", Caption = "idade", Width = Unit.Percentage(0), Visible = false });
        ...
        public override DataView GetData(IDictionary<string, object> pars, object key, int maxRows)
        {
            ...
            sql.Append("SELECT top " + maxRows.ToString() + @" d.matricula, 
                            ...
                            p.DT_NASC ,
                            REPLACE(CONVERT(int, DATEDIFF(DAY, P.DT_NASC, SYSDATETIME())/365.25),'.', ',') IDADE                                              
                         FROM LY_DOCENTE D (NOLOCK)
		                INNER JOIN LY_PESSOA P (NOLOCK) ON P.PESSOA = D.PESSOA
		                INNER JOIN LY_CATEGORIA_DOCENTE CD (NOLOCK) ON D.CATEGORIA = CD.CATEGORIA
		                INNER JOIN LY_LOTACAO L (NOLOCK) ON D.MATRICULA = L.MATRICULA AND D.PESSOA = L.PESSOA
	                    where (L.DATA_DESATIVACAO IS NULL OR L.DATA_DESATIVACAO >= CONVERT(DATE,GETDATE()))
                                AND L.DATA_NOMEACAO < CONVERT(DATE,GETDATE())
		                        AND D.CATEGORIA IN ('PROF DE SEGUNDO GRAU','PROF DOC I','PROF DOC I C REF 3','PROF DOC I C REF 4','PROF DOC I C REF 5',
	                                    'PROF DOC I C REF 6','PROF DOC I C REF 7','PROF DOC I C REF 8','PROF DOC I D REF 4','PROF DOC I D REF 5',
	                                    'PROF DOC I D REF 6','PROF DOC I D REF 7','PROF DOC I D REF 8','PROF DOC I D REF 9','PROF DOC I-12 HORAS',
	                                    'PROF DOCENTEI C REF7','PROF I CLASSE B','PROF I CLASSE C','PROF I CLASSE C NIV3','PROF I CLASSE C NIV5',
	                                    'PROF I CLASSE D NIV8')");
