...
{
    public class QueryDocente : LyceumQuery
    {
        public QueryDocente()
        {
            ...
            this.GridColumns.Add(new TSearchColumn() { FieldName = "dt_nasc", Caption = "dt_nasc", Width = Unit.Percentage(0), Visible = false });
        ...
        public override DataView GetData(IDictionary<string, object> pars, object key, int maxRows)
        {
            ...
            sql.Append("SELECT top " + maxRows.ToString() + @" d.matricula, 
		                    ...
		                    p.idfuncional, 
							convert(date,p.dt_nasc) as dt_nasc,
	                    ...
	                    FROM  ly_pessoa p (nolock) 
		                    inner join ly_docente d (nolock) on p.pessoa = d.pessoa 
	                    where voluntario = 'N'");
