...
{
    public class QueryMatriculaComIdFuncional : LyceumQuery
    {
        public QueryMatriculaComIdFuncional(): base()
        {
            ...
            this.GridColumns.Add(new TSearchColumn() { FieldName = "pessoa", Caption = "Código", Width = Unit.Percentage(12), Visible = false });
            this.GridColumns.Add(new TSearchColumn() { FieldName = "nome", Caption = "Nome", Width = Unit.Percentage(28) });
            this.GridColumns.Add(new TSearchColumn() { FieldName = "idfuncional", Caption = "Id Funcional", Width = Unit.Percentage(10) });
        ...
        public override DataView GetData(IDictionary<string, object> pars, object key, int maxRows)
        {
            ...
            sql.Append("SELECT distinct top " + maxRows.ToString() + " " +
                "p.matricula, p.pessoa,p.nome_compl as nome, p.rg_num, p.cpf, p.fone, p.celular, p.e_mail_interno,  " +
                "p.nome_mae as mae, " +
                "p.nome_pai as pai, " +
                "p.idfuncional " +
                "FROM  vw_funcionarios p where 1=1 ");

            if (key != null)
            {
                sql.Append(" and p.matricula = ? ");
