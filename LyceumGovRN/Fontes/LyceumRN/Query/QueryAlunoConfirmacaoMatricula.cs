namespace Techne.Lyceum.RN.Query
{
    ...
    public class QueryAlunoConfirmacaoMatricula : LyceumQuery
    {
        public QueryAlunoConfirmacaoMatricula()
        {
            ...
            this.GridColumns.Add(new TSearchColumn { FieldName = "unidade_ensino", Caption = "Unidade Ensino", Width = Unit.Percentage(18) });
                                            ...
                                            IF (@PRIVILEGIO = 'S') BEGIN
                                               ...
                                               FROM #USUARIO  U (NOLOCK),
                                                    ...
                                                    INNER JOIN LY_PESSOA                P (NOLOCK) ON P.PESSOA = A.PESSOA
                                                     --LEFT JOIN TCE_CONFIRMACAO_MATRICULA C (NOLOCK) ON C.ALUNO  = A.ALUNO
                                         ";                


                contextQuery.Command += " WHERE U.USUARIO = @USUARIO ";
                contextQuery.Command += contextQueryPar.Command;

                contextQuery.Command += @" END ELSE BEGIN
